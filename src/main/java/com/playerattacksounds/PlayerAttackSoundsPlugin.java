...
@Slf4j
@PluginDescriptor(
	name = "Player Attack Sounds"
)
public class PlayerAttackSoundsPlugin extends Plugin
{
	private long previousSoundTime = 0;
	private boolean newAttackPossible = true;
	private boolean isFocused = true;
	private boolean enableDamageIntake = false;
	@Inject
	...
	@Inject
	private PlayerAttackSoundsConfig config;
	...
	{
		if (!config.soundsOnDamageIntake()) return;
		{
	...
	@Provides
	PlayerAttackSoundsConfig provideConfig(ConfigManager configManager)
	{
		return configManager.getConfig(PlayerAttackSoundsConfig.class);
	}
}
